...
class BrowserifySupport {
    ...
    this.sourceTasks = [
      {
        ...
        task: pack =>
          tap((file, t) => {
            _this.browserifyOptions &&
              _this.browserifyOptions.debug &&
              log.info(`\tBrowserify: redirecting ${file.path}`);
            return t.through(dest, [
              path.join(this.intermediateDir, pack.relativePath)
            ]);
          })
      }
    ];
  ...
  addDefaultTasks(gulpTasks) {
    ...
    gulpTasks.buildSource = series(gulpTasks.buildSource, browserify);
  }
      ...
      (pack, packDone) => {
        ...
        return pump(
          [
            ...
            tap(file => {
              this.browserifyOptions &&
                this.browserifyOptions.debug &&
                log.info(`\tbundling ${path.relative(packDir, file.path)}`);
