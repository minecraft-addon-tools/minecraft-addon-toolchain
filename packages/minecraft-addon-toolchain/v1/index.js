...
class MinecraftAddonBuilder {
    ...
    parseTemplate(template, pack, packType) {
        ...
        templateVars.version = _.isArray(pack.version)
    ...
    determineMinecraftDataDirectory(done) {
        ...
        switch (os.platform()) {
            ...
            default:
                    ...
                    new Error(
                        "Unknown platform, please set the BEDROCK_DATA_DIR environment variable"
                    )
                );
                return;
        }
        ...
        fs.stat(this.gameDataDir, err => {
            if (err) {
                    ...
                    new Error(
                        "Minecraft Bedrock edition's data directory is not available: " +
                            this.gameDataDir
                    )
                );
            }
    ...
    foreachPack(description, type, action, done) {
        ...
        return series(parallel(...tasks), completeDynamicTasks)();
    }
    ...
    getTasks(selector, args) {
        return this._plugins
            ...
            .map(action => {
                const actions = [gulpIf(action.condition, action.task(args))];
        ...
        this.determinePacksInLocation(destination, installedPacks => {
            ...
            pump(
                [
                    ...
                    tap(file => {
                        log.info(
                            `\tRemoving behavior pack ${path.relative(
                                destination,
                                file.path
                            )}`
                        );
    ...
    installBehavior(done) {
        ...
        return this.foreachPack(
            ...
            "behavior",
            (pack, packDone) => {
                const packDir = this.parseTemplate(
        ...
        return this.determinePacksInLocation(destination, installedPacks => {
            ...
            return pump(
                [
                    ...
                    tap(file => {
                        log.info(
                            `\tRemoving resource pack ${path.relative(
                                destination,
                                file.path
                            )}`
                        );
    ...
    createMCPacks(done) {
        ...
        return this.foreachPack(
            "createMCPacks",
            (pack, packDone) => {
                const packZip = this.parseTemplate(
                    this.templateMcPackName,
                    pack
                );
                _internalDebug && log.info(`\t${pack.name}`);
                pump(
    ...
    createSteps() {
        const properties = Object.getOwnPropertyNames(
            MinecraftAddonBuilder.prototype
        );
        ...
        this._plugins.forEach(
            plugin => plugin.addDefaultTasks && plugin.addDefaultTasks(steps)
        );
    ...
    createTasks(steps) {
        ...
        tasks.build = series(steps.determinePacks, steps.buildSource);
        ...
        tasks.install = series(
            ...
            steps.determinePacks,
            steps.buildSource,
            parallel(steps.installBehavior, steps.installResources)
        );

        tasks.uninstall = series(
            steps.determineMinecraftDataDirectory,
            parallel(steps.cleanResources, steps.cleanBehavior)
        );
        ...
        function watchFiles() {
            ...
            const watchPath = normalize(
                path.join(path.resolve(builder.sourceDir), "**/*")
            );
