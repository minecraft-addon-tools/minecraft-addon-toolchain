...
class MinecraftModBuilder {
	constructor(modName) {
		...
		log.warn(
			"It is recommended that you update to the latest version. Please see the upgrade notes for details"
		);
		log.warn(
			"https://minecraft-addon-tools.github.io/docs/Toolchain/upgrade-notes-v1"
		);
		log.warn("------------");
	}
	...
	verifyMinecraftExists(done) {
		fs.stat(this._destRoot, err => {
			if (err) {
					...
					new Error("Minecraft Windows 10 edition is not installed")
				);
			}
			done();
		});
	...
	cleanOutDir() {
		return src(this.outDir, { read: false, allowEmpty: true }).pipe(
			clean()
		);
	}
	...
	cleanBehavior() {
		...
		return src(destination, { read: false, allowEmpty: true }).pipe(
			clean({ force: true })
		);
	}
	...
	configureEverythingForMe() {
		...
		tasks.build = parallel(tasks.scripts, tasks.behavior, tasks.resources);

		tasks.rebuild = series(function clean() {
			return builder.cleanOutDir();
		}, tasks.build);

		tasks.install = series(
			tasks.build,
			parallel(tasks.install_behaviour, tasks.install_resources)
		);
		...
		function watchFiles() {
			watchForUnlink(
				watch(
					"src/scripts/**/*",
					series(notify, tasks.scripts, tasks.install_behaviour)
				)
			);
			watchForUnlink(
				watch(
					"src/behaviors/**/*",
					series(notify, tasks.behavior, tasks.install_behaviour)
				)
			);
			watchForUnlink(
				watch(
					"src/resources/**/*",
					series(notify, tasks.resources, tasks.install_resources)
				)
			);
		}

		tasks.watch = series(tasks.clean, tasks.install, watchFiles);
...
function toArray(collection, options) {
	if (typeof collection === "string")
		return options && options.query
			? toArray(document.querySelectorAll(collection))
			: [collection];
